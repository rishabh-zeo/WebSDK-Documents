...
declare const window: Window & {
    zeotap: {
        init: (writeKey: string, options?: { areIdentitiesHashed?: boolean }) => void;
        setUserIdentities: (identities: Record<string, string>) => void;
    };
};

const UserIdentitiesHashed: React.FC = () => {
    ...
    const handleInitialize = (e: React.FormEvent) => {
        e.preventDefault();
        if (!!window.zeotap) {
            window.zeotap.init(writeKey, { areIdentitiesHashed: true });
            setInitialized(true);
        }
    };
    ...
    return (
        <div className="container">
            ...
            <div style={{ marginBottom: '1rem', fontWeight: 500, color: '#2b6cb0' }}>Already Hashed</div>
...
export default UserIdentitiesHashed; 