...
declare const window: Window & {
    zeotap: {
        init: (writeKey: string, options?: { hashIdentities?: boolean }) => void;
        setUserIdentities: (identities: Record<string, string>) => void;
    };
};

const UserIdentitiesNeedsHashing: React.FC = () => {
    ...
    const handleInitialize = (e: React.FormEvent) => {
        e.preventDefault();
        if (!!window.zeotap) {
            window.zeotap.init(writeKey, { hashIdentities: true });
            setInitialized(true);
        }
    };
    ...
    return (
        <div className="container">
            ...
            <div style={{ marginBottom: '1rem', fontWeight: 500, color: '#2b6cb0' }}>Needs Hashing</div>
...
export default UserIdentitiesNeedsHashing; 